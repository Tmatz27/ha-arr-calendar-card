...
class ArrCalendarCard extends HTMLElement {
  ...
  _normalize(raw, service) {
    ...
    return {
      ...
      poster: this._poster(images, service.key),
    };
  }

  _poster(images, serviceKey) {
    const image = images.find((entry) => entry.coverType === 'poster') || images[0];
    const url = image?.remoteUrl || image?.url;
    if (!url) return '';
    return /^https?:/i.test(url)
      ? `/api/arr_stack/${serviceKey}/image?url=${encodeURIComponent(url)}`
      : url;
  }
  ...
  _render() {
    ...
    const range = `${this._formatDate(dates[0])} – ${this._formatDate(dates[6])} ${dates[6].getFullYear()}`;
    ...
    if (this._loading) {
      ...
      content = `<section class="week ${html(this._config.item_density)}">${dates.map((date) => this._day(date, todayKey)).join('')}</section>`;
    ...
    this.shadowRoot.innerHTML = `${this._styles()}<ha-card>
      <div class="calendar" style="--calendar-height:${html(this._config.card_height)}">
        <header>
          <div class="heading"><span class="eyebrow">ARR CALENDAR</span><h2>${html(range)}</h2></div>
  ...
  _item(item) {
    ...
    return `<div class="release ${item.type}" title="${html(subtitle || item.title)}">
      ${item.poster ? `<img src="${html(item.poster)}" alt="" loading="lazy">` : '<div class="no-art">◆</div>'}
      <div class="shade"></div>
  ...
  _wireEvents() {
    ...
    this.shadowRoot.querySelectorAll('[data-action]').forEach((button) => {
      button.onclick = () => {
        ...
        if (action === 'prev') this._weekOffset -= 1;
        if (action === 'next') this._weekOffset += 1;
        if (action === 'today') this._weekOffset = 0;
        if (action === 'first') this._weekOffset -= 52;
        if (action === 'last') this._weekOffset += 52;
        this._fetchCalendar();
      };
    });
  }
  ...
  _styles() { return `<style>
    ...
    *{box-sizing:border-box}ha-card{overflow:hidden;background:var(--ha-card-background,var(--card-background-color,#fff));border-radius:var(--ha-card-border-radius,24px)}
    ...
    header{min-height:76px;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;padding:12px 20px;border-bottom:1px solid var(--divider-color,rgba(0,0,0,.12))}.heading{grid-column:2;text-align:center}.eyebrow{display:none}h2{font-size:1.05rem;margin:0;font-weight:700}nav{grid-column:3;justify-self:end;display:flex;gap:6px}
    ...
    .content{flex:1;min-height:0;padding:10px 12px}.week{height:100%;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:7px}.day{min-width:0;min-height:0;display:flex;flex-direction:column;border-radius:13px;background:color-mix(in srgb,var(--card-background-color,#fff) 88%,var(--primary-text-color) 12%);overflow:hidden}.day.today{box-shadow:inset 0 0 0 2px color-mix(in srgb,var(--primary-color,#4285f4) 70%,transparent)}.day h3{height:62px;flex:none;margin:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-transform:uppercase;border-bottom:1px solid var(--divider-color,rgba(0,0,0,.12));line-height:1.05}.day h3 span{font-size:.68rem;letter-spacing:.07em;color:var(--secondary-text-color,#777)}.day h3 b{font-size:1.4rem;margin-top:5px}.today h3{background:color-mix(in srgb,var(--primary-color,#4285f4) 23%,transparent)}
    .items{padding:7px;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin;display:flex;flex-direction:column;gap:7px;min-height:0}.empty{margin:auto;color:var(--secondary-text-color,#777);font-size:.75rem}.release{position:relative;flex:none;min-height:150px;overflow:hidden;border-radius:10px;background:#263238;border:3px solid var(--show-color);box-shadow:0 1px 3px rgba(0,0,0,.2)}.release.movie{border-color:var(--movie-color)}.release img,.no-art{position:absolute;width:100%;height:100%;inset:0;object-fit:cover}.no-art{display:grid;place-items:center;font-size:2rem;color:#90a4ae}.shade{position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,.08) 25%,rgba(0,0,0,.88) 100%)}.release-content{position:relative;z-index:1;height:100%;min-height:144px;padding:7px;display:flex;flex-direction:column;align-items:flex-start;color:#fff;text-shadow:0 1px 2px #000}.badges{width:100%;display:flex;gap:4px;align-items:center;flex-wrap:wrap}.badges span,.episode-code{background:rgba(12,18,24,.78);border-radius:5px;padding:3px 6px;font-size:.64rem;font-weight:700}.kind{border-left:3px solid var(--show-color)}.movie .kind{border-color:var(--movie-color)}.instance{margin-left:auto;color:#ddd}.count{background:color-mix(in srgb,var(--show-color) 75%,#111)!important}.episode-code{margin-top:5px;background:color-mix(in srgb,var(--show-color) 70%,#182030)}.release strong{width:100%;margin-top:auto;font-size:.78rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.release small{width:100%;font-size:.65rem;color:#e5e5e5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px}
