...
class ArrCalendarCard extends HTMLElement {
      ...
      <div class="calendar" style="--calendar-height:${html(this._config.card_height)}">
        <header>
          ...
          <nav aria-label="Calendar filters">${['all', 'shows', 'movies'].map((filter) => (
            ...
            <button class="calendar-control" data-calendar-button title="Jump to date" aria-label="Jump to date"><ha-icon icon="mdi:calendar-month"></ha-icon></button><input class="date-picker" type="date" aria-label="Jump to date" data-start-date value="${this._dateKey(dates[0])}">
          </nav>
        </header>
  ...
  _wireEvents() {
    ...
    const calendarButton = this.shadowRoot.querySelector('[data-calendar-button]');
    if (calendarButton && startDate) calendarButton.onclick = () => {
      if (typeof startDate.showPicker === 'function') startDate.showPicker();
  ...
  _styles() { return `<style>
    ...
    *{box-sizing:border-box}ha-card{overflow:hidden;background:var(--ha-card-background,var(--card-background-color,#fff));border-radius:var(--ha-card-border-radius,24px)}
    ...
    button{font:inherit;color:inherit;cursor:pointer}.pill,.round,.days-control,.calendar-control{border:1px solid var(--divider-color,rgba(0,0,0,.15));background:color-mix(in srgb,var(--card-background-color,#fff) 88%,var(--primary-text-color) 12%);box-shadow:0 1px 2px rgba(0,0,0,.04)}.pill{padding:7px 14px;border-radius:999px;font-size:.84rem;font-weight:650}.pill:hover,.pill.active{border-color:color-mix(in srgb,var(--primary-color,#4285f4) 60%,transparent);background:color-mix(in srgb,var(--primary-color,#4285f4) 24%,var(--card-background-color,#fff));color:var(--primary-text-color,#202124)}.days-control{position:relative;display:flex;align-items:center;gap:5px;min-height:36px;padding:5px 10px 5px 13px;border-radius:999px;font-size:.78rem;font-weight:650;cursor:pointer}.days-control strong{font-size:.9rem}.days-control ha-icon{width:17px;height:17px}.days-control select{position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer}.calendar-control{width:36px;height:36px;display:grid;place-items:center;border-radius:50%;padding:0}.calendar-control ha-icon{width:20px;height:20px;color:var(--secondary-text-color,#777)}.date-picker{position:fixed;left:-100vw;top:-100vh;width:1px;height:1px;opacity:0}.days-control:hover,.calendar-control:hover{border-color:color-mix(in srgb,var(--primary-color,#4285f4) 60%,transparent);background:color-mix(in srgb,var(--primary-color,#4285f4) 24%,var(--card-background-color,#fff))}.calendar-control:hover ha-icon{color:var(--primary-text-color,#222)}
