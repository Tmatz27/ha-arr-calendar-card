...
class ArrCalendarCard extends HTMLElement {
  ...
  _poster(images, serviceKey) {
    ...
    if (!mediaCover) return '';
    return `/api/arr_stack/${serviceKey}/api/v3${mediaCover[1]}`;
  }
  ...
  _item(item) {
    ...
    return `<div class="release ${item.type}" title="${html(subtitle || item.title)}">
      ...
      ${item.poster ? `<img src="${html(item.poster)}" alt="${html(item.title)} poster" loading="lazy">` : ''}
      <div class="shade"></div>
